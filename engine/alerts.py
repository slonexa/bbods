...
class AlertManager:
    ...
    def process_spreads(self, spreads: List[Dict[str, Any]]):
        """Evaluate spreads and fire alerts for high-value opportunities."""
        ...
        for s in spreads:
            ...
            if is_arb and self.config.get("notify_on_surebet", True) and margin_pct >= min_margin_pct:
                should_alert = True
                alert_type = "SUREBET"
            elif self.config.get("notify_on_high_spread", True) and spread_pct >= min_spread_pct:
                should_alert = True
            ...
            # Check cooldown
            last_alert_time = self.cooldown_tracker.get(event_key, 0)
            if (now_ts - last_alert_time) < cooldown_sec:
                continue

            # Record alert
            self.cooldown_tracker[event_key] = now_ts
            alert_payload = {
